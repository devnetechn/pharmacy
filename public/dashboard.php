...
foreach ($drugs as $d) {
    ...
    if ($bal <= (int)$d['reorder_level']) {
        $low[] = ['name' => $d['name'], 'balance' => $bal, 'reorder' => (int)$d['reorder_level']];
    }
}

// near-expiry: received_in batches expiring within 30 days
$expStmt = $pdo->query(
    "SELECT d.name, t.expiry_date, t.quantity
...
<?php if ($low): ?>
  <?php foreach ($low as $l): ?>
    <div class="alert danger"><?= htmlspecialchars($l['name']) ?>: balance <?= $l['balance'] ?> (reorder at <?= $l['reorder'] ?>)</div>
  <?php endforeach; ?>
...
<?php if ($expiring): ?>
  <?php foreach ($expiring as $e): ?>
    <div class="alert"><?= htmlspecialchars($e['name']) ?> &mdash; expires <?= htmlspecialchars($e['expiry_date']) ?> (qty <?= (int)$e['quantity'] ?>)</div>
  <?php endforeach; ?>
